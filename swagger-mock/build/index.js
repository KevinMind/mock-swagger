(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('events'), require('path'), require('util'), require('tty'), require('fs'), require('net'), require('buffer'), require('string_decoder'), require('stream'), require('zlib'), require('querystring'), require('url'), require('http'), require('crypto')) :
	typeof define === 'function' && define.amd ? define(['events', 'path', 'util', 'tty', 'fs', 'net', 'buffer', 'string_decoder', 'stream', 'zlib', 'querystring', 'url', 'http', 'crypto'], factory) :
	(global = global || self, factory(global.events, global.path$2, global.util, global.tty, global.fs, global.net, global.buffer, global.string_decoder, global.stream, global.zlib, global.querystring, global.url, global.http, global.crypto));
}(this, function (events, path$2, util, tty, fs, net, buffer, string_decoder, stream, zlib, querystring, url, http, crypto) { 'use strict';
	...
	function SBCSCodec$1(codecOptions, iconv) {
	    ...
	    this.decodeBuf = new Buffer$c.from(codecOptions.chars, 'ucs2');
	    
	    // Encoding buffer.
	    var encodeBuf = new Buffer$c.alloc(65536, iconv.defaultCharSingleByte.charCodeAt(0));
	...
	res.send = function send(body) {
	  ...
	  switch (typeof chunk) {
	    ...
	    case 'object':
	      if (chunk === null) {
	        chunk = '';
	      } else if (Buffer$g.isBuffer(chunk)) {
	        if (!this.get('Content-Type')) {
	          this.type('bin');
	        }
	      } else {
	  ...
	  var len;
	  if (chunk !== undefined) {
	    if (Buffer$g.isBuffer(chunk)) {
	      // get length of Buffer
	      len = chunk.length;
	    } else if (!generateETag && chunk.length < 1000) {
	      // just calculate length when no ETag + small chunk
	      len = Buffer$g.byteLength(chunk, encoding);
	    } else {
	...
	res.redirect = function redirect(url) {
	  ...
	  this.set('Content-Length', Buffer$g.byteLength(body));
